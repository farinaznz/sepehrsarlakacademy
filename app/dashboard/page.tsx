...
export default async function DashboardPage() {
  ...
  const rows = await db.select({
    ...
    lessonId: lesson.id, lessonSlug: lesson.slug, lessonTitle: lesson.title,
  ...
  const enrolledCourses = new Map<string, {
    ...
    lessons: Array<{ id: string; slug: string; title: string; percent: number; available: boolean; releaseAt: Date; lastViewedAt: Date | null }>;
  }>();
  for (const row of rows) {
    ...
    if (row.lessonId && row.lessonSlug && row.lessonTitle && row.dripDelayDays !== null) {
      entry.lessons.push({
        id: row.lessonId, slug: row.lessonSlug, title: row.lessonTitle, percent: row.progress ?? 0,
  ...
  return <section className="student-area section"><div className="container">
    ...
    {enrolledCourses.size ? <div className="student-course-grid">{[...enrolledCourses.values()].map((item) => {
      ...
      return <article className="student-course-card" key={item.slug}>
        ...
        <div className="student-lesson-list">{item.lessons.map((entry) => entry.available ?
